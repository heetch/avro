...
func (e *compatibilityError) Error() string {
	return fmt.Errorf("incompatible at %s: %v", e.path, e.msg).Error()
}

func compatErrorf(path string, f string, args ...interface{}) error {
	return &compatibilityError{
		path: path,
		msg:  fmt.Errorf(f, args...).Error(),
	}
}
...
func checkGoCompatible(path string, t1, t2 schema.AvroType, checked map[schema.AvroType]bool) error {
	...
	case *schema.UnionField:
		...
		for i := range itemTypes1 {
			if err := checkGoCompatible(path+fmt.Errorf("[u%d]", i).Error(), itemTypes1[i], itemTypes2[i], checked); err != nil {
				return err
			}
		}
